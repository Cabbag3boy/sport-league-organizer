...
const RankChangeDisplay: React.FC<{ player: Player; oldRank: number | undefined; newRank: number }> = ({ player, oldRank, newRank }) => {
    ...
    return (
        <div className="flex justify-between items-center text-sm">
            ...
            <div className="flex items-center gap-2">
                ...
                <span>→</span>
                <span className={`${colorClass} font-bold`}>{newRank} {symbol}</span>
            </div>
        </div>
    );
};

const StaticMatch: React.FC<{ player1Name: string; player2Name: string; score1: string; score2: string; }> = ({ player1Name, player2Name, score1, score2 }) => {
...
const RoundHistoryCard: React.FC<RoundHistoryCardProps> = ({ entry, isSelected, onToggleSelection }) => {
  ...
  const rankChanges = useMemo(() => {
    ...
    return entry.playersAfter
      ...
      .filter(change => change.oldRank !== undefined && change.oldRank !== change.newRank);
  ...
  const renderMatchesFromHistory = (group: Player[], groupIndex: number) => {
    ...
    if (group.length === 4) {
      ...
      let winner1Name = "Winner (R1 M1)";
      let loser1Name = "Loser (R1 M1)";
      ...
      let winner2Name = "Winner (R1 M2)";
      let loser2Name = "Loser (R1 M2)";
      ...
      return (
        <div className="space-y-3">
            <div>
              <h6 className="font-semibold text-gray-400 mb-1 text-xs">Round 1</h6>
              <div className="space-y-1">
                  <StaticMatch player1Name={p1.name} player2Name={p4.name} score1={r1m1Scores.score1} score2={r1m1Scores.score2} />
                  <StaticMatch player1Name={p2.name} player2Name={p3.name} score1={r1m2Scores.score1} score2={r1m2Scores.score2} />
              </div>
            </div>
            <div>
              <h6 className="font-semibold text-gray-400 mb-1 text-xs">Round 2</h6>
              <div className="space-y-1">
                <StaticMatch player1Name={winner1Name} player2Name={winner2Name} score1={r2m1Scores.score1} score2={r2m1Scores.score2} />
                <StaticMatch player1Name={loser1Name} player2Name={loser2Name} score1={r2m2Scores.score1} score2={r2m2Scores.score2} />
              </div>
            </div>
        </div>
      );
    ...
    if (group.length === 3) {
      ...
      return (
        <div className="space-y-1">
          <StaticMatch player1Name={p1.name} player2Name={p2.name} score1={m1Scores.score1} score2={m1Scores.score2} />
          <StaticMatch player1Name={p1.name} player2Name={p3.name} score1={m2Scores.score1} score2={m2Scores.score2} />
          <StaticMatch player1Name={p2.name} player2Name={p3.name} score1={m3Scores.score1} score2={m3Scores.score2} />
        </div>
      );
    }
    return null;
    ...
    <div className="bg-gray-800 rounded-lg overflow-hidden transition-all duration-300">
      ...
      >
        <div className="flex items-center gap-3">
          <input
            ...
            aria-label={`Select round of ${entry.date}`}
          />
          <span className="font-semibold text-indigo-400">Round of {entry.date}</span>
        </div>
        <svg
      ...
      {isExpanded && (
        <div className="p-4 space-y-6">
          <div>
            <h4 className="text-lg font-semibold mb-2 text-indigo-400">Matches & Scores</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {entry.groups.map((group, index) => (
                <div key={index} className="bg-gray-900 p-3 rounded-md">
                  <h5 className="font-bold text-gray-300 mb-2">Group {index + 1}</h5>
          ...
          <div>
            <h4 className="text-lg font-semibold mb-2 text-indigo-400">Final Standings</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {rankChanges.length > 0 && (
            <div>
              <h4 className="text-lg font-semibold mb-2 text-indigo-400">Rank Changes</h4>
              <div className="bg-gray-900 p-3 rounded-md space-y-2">
                {rankChanges.map(({ player, oldRank, newRank }) => (
                  <RankChangeDisplay key={player.id} player={player} oldRank={oldRank} newRank={newRank} />
                ))}
              </div>
            </div>
          )}
